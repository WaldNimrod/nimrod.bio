 ...
 *
 * TLS NOTE (all uPress *.upress.link dev URLs):
 *   Dev has an INVALID/expired cert (valid SSL exists only on the primary domain).
 *   Chrome is launched with --ignore-certificate-errors. This is a DEV-ONLY flag;
 *   production (primary domain) has a valid cert and must NOT use it. See
 *   docs/QA_HARNESS.md + CLAUDE.md "uPress hosting".
 *
 * USAGE:
...
async function main() {
  ...
  const proc = spawn(chrome, [
    ...
    '--ignore-certificate-errors', // DEV ONLY — uPress *.upress.link invalid cert
    '--hide-scrollbars',
  ], { stdio: 'ignore' });
